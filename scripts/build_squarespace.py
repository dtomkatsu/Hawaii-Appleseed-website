...
def build_homepage(lines):
    ...
    css += (
        ...
        "   the top). The film's height cap reserves only 150px so that on\n"
        "   wide+tall screens 54vw wins the min() and the film renders full\n"
        "   size; the cap binds ONLY on short viewports, shrinking the film\n"
        "   there so the mission wave (60px overhang) never laps it. offset\n"
        "   -0.6rem nudges the film UP: less top whitespace, more wave gap.\n"
        "   Verified 1999x960 -> full 54vw, 30px wave clear; 1999x760 ->\n"
        "   cap active, 25px clear, no overlap. */\n"
        ...
        "    width: min(54vw, calc((80svh - 150px) * 16 / 9)) !important;\n"
        "  }\n"
        "}\n"
